...
//
// ---------------------------------------------------------------------------
// 선물 WS 주소에는 /market이 반드시 들어가야 한다. 레거시 주소는 조용히 죽는다.
//
// 바이낸스는 2026-04-23부로 선물 WS의 레거시 주소를 폐지했다.
//
//   구) wss://fstream.binance.com/ws          ← aggTrade가 오지 않는다
//   신) wss://fstream.binance.com/market/ws   ← aggTrade는 /market 소속이다
//
// 고약한 점은 실패하는 방식이다. 레거시 주소는 없어진 게 아니라 남아 있고,
// TCP·TLS·WS 핸드셰이크가 전부 성공하며, SUBSCRIBE를 보내면 성공 응답
// ({"result":null,"id":1})까지 돌려준다. LIST_SUBSCRIPTIONS로 물어보면
// 구독 목록에 멀쩡히 들어 있다. 그런데 데이터만 영원히 오지 않는다.
//
// 그래서 감지기 입장에서는 "연결됨" 로그가 찍히고, 매 초 시계가 돌고,
// 평가 카운터가 올라가는데, 거래대금만 계속 0이다. 창이 조용해 보일 뿐이라
// 알림이 안 나가는 게 시장이 잠잠해서인지 스트림이 죽어서인지 구분되지 않는다.
//
// 실제로 이 프로젝트는 선물로 전환한 2026-08-04부터 이틀 넘게 실시간 감지가
// 통째로 죽어 있었고, 그동안 DB에 남은 알림은 전부 부팅 직후 백필 데이터로
// 나간 것이었다. 부팅 로그는 내내 정상으로 보였다.
//
// 그 이틀을 되풀이하지 않으려고 SymbolAggregator가 체결 수를 세고
// (ingestDiagnostics), 스트림이 붙은 뒤에도 체결이 없으면 경고를 낸다.
// ---------------------------------------------------------------------------
