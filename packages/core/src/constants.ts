...
/** 기본값은 15분봉급 규모에 맞춘다 (슬라이더 36, 코인당 하루 3~4회). */
export const SENSITIVITY_DEFAULT = 99.887;
 ...
 * 규모별로 채널 신호(프레임 최대 백분위)의 중앙값을 구했다.
 * 짧게 터지고 마는 급등은 1분 창만 흔들어 신호가 약하고, 크고 오래 가는
 * 급등은 1일 창까지 흔들어 신호가 강하다.
 *
 * 그래서 1분봉급이 가장 오른쪽(높은 민감도라야 잡힘), 1일봉급이 가장
 * 왼쪽(낮은 민감도로도 잡힘)에 온다.
 */
...
export const CHANNEL_RATE_CURVE: readonly number[] = [
  0.84, 1.01, 1.23, 1.59, 2.05, 2.72, 3.39, 4.41, 5.55, 7.06, 8.92, 11.21, 13.9,
  17.15, 20.97, 24.84, 29.08, 33.36, 37.55, 41.66,
];
