...
export interface Channel {
   ...
   * 민감도. 1~100 슬라이더 위치다.
   *
   * 판정에 필요한 두 값(창 길이와 배수)이 여기서 함께 나온다
   * (sensitivityAt). 20칸씩 다섯 구간이고 구간마다 창 길이가 붙으며,
   * 구간 안에서는 배수가 연속으로 움직인다. 클수록 잦다.
   *
   * 창 길이를 따로 저장하지 않는 이유: 두 값을 다 저장하면 서로 어긋날
   * 수 있고, 어긋난 순간 화면에 적힌 빈도가 실제와 달라진다.
   */
  sensitivityLevel: number;
  /**
   * @deprecated 다섯 칸이던 시절의 창 길이. 연속 축으로 옮기기 전에
   * 저장된 값이다. 읽어서 sensitivityLevel로 환산하는 데만 쓴다
   * (levelForScale).
   */
  scale?: Timeframe;
  /**
